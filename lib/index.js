...
function routerStrategy(metadata, dataAdapter, opts) {
    ...
    for (const name in metadata) {
        ...
        for (const key in routes) {
            if (routeSwitch[key] !== true) continue;
...
function middlewareStrategy(metadata, dataAdapter, opts) {
    ...
    return async function magic(ctx, next) {
        ...
        let middleware;
        if (routeSwitch[key] === true) {
            const { pres, posts } = getHooks(obj, key);
            const route = routes[key];
            middleware = route.call(null, name, dataAdapter, opts, pres, posts);
        }
    ...
    switch (method) {
        case 'GET':
            ...
            if (reg.test(pathname)) {
                // 如果匹配成功，则说明url是：/param/xx这样的结构，目前这种结构只有id查询
                key = 'id';
            } else {
                // 如果匹配失败，则说明url是：/param这样的结构，目前这种结构只有list查询
                key = 'list';
            }
            break;
        case 'POST':
